...
char *concat_str(char *a, char *b, char *c)
{
	char *name;


}
// viet them doc training data and label

vector<case_t> read_data(char *name, int num_of_case, int num_in_person, int weight, int height, int num_of_person)
{
	vector<case_t> cases;
...
void print_text(vector<case_t> cases, int amount, const char* name){
	FILE *myfile = fopen(name, "w");
	int num = 0;
	for ( case_t& t : cases ){
		fprintf(myfile, "[case %d]\n", num++);
		print_text_tensor(t.data, myfile);
		print_text_tensor(t.out, myfile);
		fprintf(myfile, "\n===========================\n");
		if (num == amount) break;
	}
	fclose(myfile);
...
int main()
{
	...
	print_text(cases, 3, "read_test_cases.txt"); 
	return 0;
